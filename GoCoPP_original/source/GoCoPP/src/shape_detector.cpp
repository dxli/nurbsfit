...
void Shape_Detector::to_vg(std::string & out_dir)
{
	int X = 1, Y = 1, Z = 1;
	Pwn_vector T = points;
	// Step 1.
	...
	for (int i = 0; i < X; ++i) {
		for (int j = 0; j < Y; ++j) {
			for (int k = 0; k < Z; ++k) {
				...
				std::string filename = out_dir + path_point_cloud_basename + "_planar_primitives_detection.vg";
...
bool Shape_Detector::load_ply()
{
  ...
  if (!stream || !CGAL::IO::read_PLY(stream, std::back_inserter(points),
...
void Shape_Detector::save_convex_hull(std::string & out_dir) {
	...
	std::string filename = out_dir + path_point_cloud_basename + "_convex_hull.ply";
	std::ofstream stream(filename, std::ios::out);
	if (!stream.is_open()) {
...
void Shape_Detector::save_alpha_shapes(std::string & out_dir) {
	std::vector<std::vector<Inexact_Point_3> > as_points;
	std::vector<CGAL::Color> as_colors;
	for (size_t p = 0; p < convex_hulls_pts.size(); ++p) {
		if (convex_hulls_pts[p].size()<3) continue;
	...
	std::string filename = out_dir + path_point_cloud_basename + "_alpha_shape.ply";
	std::ofstream stream(filename, std::ios::out);
	if (!stream.is_open()) {
		throw std::ios_base::failure("Error : cannot write into an output file");
	}
...
void Shape_Detector::save_primitive_conection(const std::string & filename) {
    ...
    for (int i = 0; i < primitive_connection.size(); ++i) {
        ...
        for (int j = 0; j < primitive_connection.size(); ++j) {
            if (primitive_connection[i][j] == 1) {
                indices.push_back(j);
            }
        }
        // Write the number of adjacent elements followed by the indices
...
void Shape_Detector::save_metrics(const std::string & filename) {
	...
	outfile << "npoints " << this->points.size() << "\n";
	outfile << "epsilon " << this->epsilon << "\n";
	outfile << "bbox_diagonal " << this->bbox_diagonal << "\n";
	outfile << "KNN " << this->knn << "\n";
	outfile << "normal_threshold " << this->normal_threshold << "\n";

	outfile << "primitives " << (this->primitives_number) << "\n";
	outfile << "coverage " << (this->coverage) << "\n";
	outfile << "mean_error " << (this->mean_error) << "\n";
	outfile << "mean_normal_deviation " << (this->mean_normal_diviation) << "\n";
...
void Shape_Detector::detect_planes()
{
	...
	for (size_t i = 0; i < planes_to_inliers.size(); ++i) {
		...
		r = u_char(uniform_distribution(generator));
		g = u_char(uniform_distribution(generator));
		b = u_char(uniform_distribution(generator));
